...
class OmniRobotRLController(Node, gym.Env):
    def __init__(self, reward_type='full'):
        ...
        self.csv_filename = f"ppo_v2_pid_behavior_{timestamp}.csv"
        self.csv_file = open(self.csv_filename, 'w', newline='')
        self.csv_writer = csv.writer(self.csv_file)
        self.csv_writer.writerow(['Step', 'Action', 'Dist', 'Rew', 'R_Prog', 'R_Align', 'R_Drift'])
        self.get_logger().info(f"Logging V2 data to: {self.csv_filename}")

        # Spaces
        # State: [dist, cos(head), sin(head), vx, vy, wz]
        self.observation_space = spaces.Box(low=-np.inf, high=np.inf, shape=(6,), dtype=np.float32)
        # 11 Discrete Actions (Stop, Cardinal, Diagonals, Rotations)
    ...
    def step(self, action):
        ...
        # Calculates if the velocity vector is perfectly aligned with the goal vector
